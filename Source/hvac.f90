...
REAL(EB) :: AREA,DIAMETER,XYZ(3),LOSS(MAX_DUCTS,MAX_DUCTS),VOLUME_FLOW,MAX_FLOW,MAX_PRESSURE,ROUGHNESS,LENGTH,TNOW,TAU_AC,&
            ...
            COOLANT_MASS_FLOW,COOLANT_SPECIFIC_HEAT,COOLANT_TEMPERATURE,PERIMETER,MASS_FLOW,SYSTEM_LOSS
REAL(EB) :: LOADING(MAX_SPECIES),EFFICIENCY(MAX_SPECIES),LOADING_MULTIPLIER(MAX_SPECIES)
LOGICAL :: ROUND, SQUARE, DAMPER, REVERSE, AMBIENT,LEAK_ENTHALPY,INITIALIZED_HVAC_MASS_TRANSPORT,QFAN_BETA=.FALSE.,QFAN_BETA2=.FALSE.
...
NAMELIST /HVAC/ AIRCOIL_ID,AMBIENT,AREA,CLEAN_LOSS,COOLANT_SPECIFIC_HEAT,COOLANT_MASS_FLOW,COOLANT_TEMPERATURE,CTRL_ID,&
                ...
                MASS_FLOW,MAX_FLOW,MAX_PRESSURE,N_CELLS,NODE_ID,PERIMETER,QFAN_BETA,QFAN_BETA2,SYSTEM_LOSS,&
...
DO NN=1,N_HVAC_READ
   ...
   SELECT CASE (TYPE_ID)
      ...
      CASE('FAN')
         ...
         ELSEIF(QFAN_BETA2 .EQV. .TRUE.) THEN
            FAN(I_FAN)%FAN_TYPE = 5
         ELSE
...
USE GLOBAL_CONSTANTS, ONLY: GRAV
USE PHYSICAL_FUNCTIONS,ONLY: GET_VISCOSITY
INTEGER :: FAN_ITER,IPZ
INTEGER, INTENT(IN) :: DUCT_INDEX
REAL(EB), INTENT(IN) :: T
REAL(EB) :: TSI,FLOW1,FLOW2,FUNC,FLOWGUESS,ZZ_GET(1:N_TRACKED_SPECIES),FRICTION_FACTOR,VISCOSITY,HEAD,LOSS,ZONEP(2)=0._EB,&
            del_p1,del_p2,guessup,guessdown
TYPE(DUCT_TYPE), POINTER::DU=>NULL()
...
SELECT CASE (FA%FAN_TYPE)
   ...
   CASE(4) ! System curve-based quadratic fan BETA
      ...
      FAN_LOOP1: DO
         ...
         IF (ABS(FLOWGUESS - FLOW1) < TWO_EPSILON_EB) EXIT FAN_LOOP1
         ...
         FUNC = FUNC - FA%MAX_PRES + FA%MAX_PRES*(FLOWGUESS/FA%MAX_FLOW)**2 ! i think this should be a minus not a plus
         IF (FUNC > 0) THEN
            FLOW2 = FLOWGUESS
         ELSE
            FLOW1 = FLOWGUESS
      ...
      DEL_P = DEL_P*EVALUATE_RAMP(TSI,FA%TAU,FA%SPIN_INDEX)
   CASE(5) ! Local system-curve based quadratic fan BETA
      ! FLOWGUESS = ABS(DU%VEL(PREVIOUS)+DU%VEL(NEW))*0.5_EB*DU%AREA
      ! FAN_ZERO_DENOM_IF: IF (FLOWGUESS < (0.1_EB*FA%MAX_FLOW)) THEN ! flow is less than 10% of max flow; use default quadratic model
      !    print*,'using quad model. iteration number:',ITER
      !    VDOT = (0.25_EB*DU%VEL(NEW)+0.25_EB*DU%VEL(PREVIOUS)+0.5_EB*DU%VEL(OLD))*DU%AREA
      !    IF (DU%REVERSE) VDOT = -VDOT
      !    VDOT = MAX(0._EB,MIN(VDOT,FA%MAX_FLOW))
      !    DEL_P = FA%MAX_PRES - FA%MAX_PRES*(VDOT/FA%MAX_FLOW)**2
      !    DEL_P = DEL_P*EVALUATE_RAMP(TSI,FA%TAU,FA%SPIN_INDEX)
      ! ELSE
         DN1=>DUCTNODE(DU%NODE_INDEX(1))
         DN2=>DUCTNODE(DU%NODE_INDEX(2))
         ! Get two points on system curve
         HEAD = DN1%P-DN2%P+DU%RHO_D*GRAV*(DN1%XYZ(3)-DN2%XYZ(3))
         IF (DU%SYSTEM_LOSS > 0) THEN ! user input
            LOSS = 0.5_EB*DU%RHO_D*DU%SYSTEM_LOSS
         ELSE
            LOSS = 0.5_EB*DU%RHO_D*DU%TOTAL_LOSS
         ENDIF
         ! print*,'iteration',ITER
         ! print*,'duct p1 and p2:',DN1%P,DN2%P
         IF (HEAD/LOSS > 0) THEN
            FLOW1 = SQRT(HEAD/LOSS)*DU%AREA
         ELSE
            FLOW1 = SQRT(-HEAD/LOSS)*DU%AREA
         ENDIF
         IF ((HEAD + FA%MAX_PRES)/LOSS > 0) THEN
            FLOW2 = SQRT((HEAD + FA%MAX_PRES)/LOSS)*DU%AREA
         ELSE
            FLOW2 = SQRT(-(HEAD + FA%MAX_PRES)/LOSS)*DU%AREA
         ENDIF
         ! print*,'flow1 and flow2 (system curve):',FLOW1,FLOW2
         DEL_P1 = FA%MAX_PRES-FA%MAX_PRES*(FLOW1/FA%MAX_FLOW)**2
         del_p2 = FA%MAX_PRES-FA%MAX_PRES*(FLOW2/FA%MAX_FLOW)**2
         ! Use bisect to find intersection point (operation point)
         FAN_ITER = 0
         guessdown = 0._EB
         guessup = FA%MAX_FLOW
         FAN_LOOP2: DO
            FAN_ITER = FAN_ITER + 1
            FLOWGUESS = 0.5*(GUESSDOWN+GUESSUP)
            IF (ABS(FLOWGUESS - GUESSDOWN) < TWO_EPSILON_EB) EXIT FAN_LOOP2
            FUNC = FLOWGUESS**2*(FLOW1-FLOW2)**2+FA%MAX_FLOW**2*(FLOWGUESS-FLOW1)**2-&
                   FA%MAX_FLOW**2*(FLOW1-FLOW2)**2
            IF (FUNC > 0) THEN
               IF (ABS(FLOWGUESS - GUESSUP)/FLOWGUESS < 0.0001) EXIT FAN_LOOP2
               GUESSUP = FLOWGUESS
            ELSE
               IF (ABS(FLOWGUESS - GUESSDOWN)/FLOWGUESS < 0.0001) EXIT FAN_LOOP2
               GUESSDOWN = FLOWGUESS
            ENDIF
            IF (FAN_ITER > 100) THEN
               FLOWGUESS = 0.5_EB*(GUESSDOWN + GUESSUP)
               EXIT FAN_LOOP2
            ENDIF
         ENDDO FAN_LOOP2
         ! print*,'iterations required:',FAN_ITER
         ! print*,'final flowguess:',flowguess
         ! Output pressure at operation point
         DEL_P = FA%MAX_PRES-FA%MAX_PRES*(FLOWGUESS/FA%MAX_FLOW)**2
         ! print*,'final del_p pre ramp:',del_p
         DEL_P = DEL_P*EVALUATE_RAMP(TSI,FA%TAU,FA%SPIN_INDEX)
         ! print*,'final del_p post ramp:',del_p
         ! read *
      ! ENDIF FAN_ZERO_DENOM_IF
END SELECT
...
INTEGER :: NN,NR,NN2,NN3,NN4,ND,DUCT_COUNTER(N_DUCTS),NODE_COUNTER(N_DUCTNODES),&
           NODE_CHECKED(N_DUCTNODES),CHKFLG,NODE_CONNECTED(N_DUCTNODES),N_QFANS,N_Q2FANS
